...
def main():
    ...
    parser.add_argument("-t", nargs='+',help="millisecond of the image to be extracted", type=float)
    ...
    tlist = [rospy.Time().from_sec(ti) for ti in args.t]

    #extracting images in 1 pass
    for ti in tlist:
        #print "Extracting Image at %.2fs" % (ti.to_nsec()/1e9)
        for topicstr in [args.left , args.right]:
            ptr = bag.read_messages(topics=topicstr,start_time=ti)
        
            topic, msg, t = next(ptr)
            print topic, " at ", (t).to_sec()
            cv_img = bridge.imgmsg_to_cv2(msg, desired_encoding="passthrough")
            cv2.imwrite(os.path.join(args.output_dir, "%08i%s.png" % ((t).to_sec()*1e3,topic.replace('/','_'))), cv_img)
    ...
    for li in args.l or []:
        #with open(os.path.join(args.output_dir,"%s.csv" % topic.replace('/','_')),'w') as outcsv:
            ...
            for ti in tlist:
                ...
                ptr = bag.read_messages(topics=[li],start_time= ti)
                try:
                    topic, msg, t = next(ptr)
                    ...
                    continue
                print topic, " at ", (t).to_sec()
                fd = open(os.path.join(args.output_dir,"%08i%s.txt" % (((t).to_sec()*1e3),topic.replace('/','_'))),'w')
                fd.write("%s\n" % msg)
